...
describe Puppet::Type.type(:par).provider(:par) do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      describe '#exists?' do
        before(:each) do
          allow(File).to receive(:exist?).with(playbook_path).and_return(true)
      ...
      describe '#create' do
        ...
        context 'when in noop mode' do
          ...
          it 'logs what would be executed' do
            allow(provider).to receive(:build_command).and_return(['ansible-playbook', '-i', 'localhost,', playbook_path])
      ...
      describe '#build_command' do
        ...
        it 'includes the playbook path' do
          expect(provider.build_command).to include(playbook_path)
        end

        it 'places the playbook path at the end' do
          expect(provider.build_command.last).to eq(playbook_path)
        end
        ...
        context 'with playbook_vars parameter' do
          ...
          it 'places vars before the playbook path' do
            command = provider.build_command
            e_index = command.index('-e')
            playbook_index = command.index(playbook_path)
